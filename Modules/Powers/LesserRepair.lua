...
function Name(unit) -- Mandatory function
	return 'Units Lesser Repair'
end

function IsAvailable(unit) -- Mandatory function
	local id = unit:GetEntityId()
	local BaseClass =  DM.GetProperty(id,'BaseClass')
	if CF.IsMilitary(unit) and DM.GetProperty(id,'PrestigeClassPromoted', nil) == 1 and BaseClass == 'Support' then return true end
	return false
end
...
function Description(unit) -- Mandatory function
	...
	table.insert(Tp.Line, {'1 Cycle (5 unit / s) (AOE 15)', Color.AQUA})
