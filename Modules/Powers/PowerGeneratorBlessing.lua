...
function Description(unit) -- Mandatory function
	...
	local Power = math.pow(bp.Economy.BuildCostMass, 0.5) * (0.75 + Int/50 + Bardsong/25) * PowerModifier
	...
	if TechLevel >= 1 then
		table.insert(Tp.Line, {'..Tech 1 powers generators :', Color.GREY_LIGHT})
		table.insert(Tp.Line, {'......Energy Production Rate +'..ProductionBonus..' %', Color.AEON})
	end
	if TechLevel >= 2 then
		table.insert(Tp.Line, {'..Tech 2 powers generators :', Color.GREY_LIGHT})
		table.insert(Tp.Line, {'......Energy Production Rate +'..ProductionBonus2..' %', Color.AEON})
	end
	if TechLevel >= 3 then
		table.insert(Tp.Line, {'..Tech 3 powers generators :', Color.GREY_LIGHT})
		table.insert(Tp.Line, {'......Energy Production Rate +'..ProductionBonus3..' %', Color.AEON})
	end
	table.insert(Tp.Line, {''})
	if DM.GetProperty(id, 'CurrentBardsong') == 'PowerGeneratorBlessing' then
...
function OnCast(unit, TempEntity, Option) -- Mandatory function. This function is called from ui immediately when the power icon is clicked on
	...
	if DM.GetProperty(id, 'CurrentBardsong') ~= 'PowerGeneratorBlessing' then
		...
		DM.SetProperty(id, 'CurrentBardsong', 'PowerGeneratorBlessing')
...
BardSongBuff = function(unit)
	...
	repeat
		...
		local Power = math.pow(bp.Economy.BuildCostMass, 0.5) * (0.75 + Int/50 + Bardsong/25) * PowerModifier
		local time = math.floor(GetGameTimeSeconds())
		local units = unit:GetAIBrain():GetUnitsAroundPoint(categories.ENERGYPRODUCTION, unit:GetPosition(), 20, 'Ally')
	...
	until(unit.Dead == true or DM.GetProperty(id, 'CurrentBardsong') ~= 'PowerGeneratorBlessing')
end
...
ProductionBuff = function(unit, _ProductionBonus)
	...
	if unit:HasEnhancement('ResourceAllocation') then
		BuffBlueprint {
			Name = 'PowerGeneratorBlessing',
			DisplayName = 'PowerGeneratorBlessing',
			BuffType = 'Bardsong',
			Stacks = 'IGNORE',
			Duration = 5,
			Affects = {
				EnergyProduction = {
					Add = (_ProductionBonus/100) * 50 + EnergyProduction * (_ProductionBonus/100),
				},
			},
		}
		Buff.ApplyBuff(unit, 'PowerGeneratorBlessing')
	else
		BuffBlueprint {
			Name = 'PowerGeneratorBlessing',
			DisplayName = 'PowerGeneratorBlessing',
			...
			Affects = {
				EnergyProduction = {
					Add = EnergyProduction * (_ProductionBonus/100) + (_ProductionBonus/100),
				},
			},
		}
		Buff.ApplyBuff(unit, 'PowerGeneratorBlessing')
