...
function Description(unit) -- Mandatory function
	...
	table.insert(Tp.Line, {'...... Movement Rate +'..SpecializationBonus..' %', Color.AEON})
	local UnitTypes = CF.GetUnitTypes(unit)
	table.insert(Tp.Line, {'..Mobile Units up to Tech '..TechLevel, Color.GREY_LIGHT})
	table.insert(Tp.Line, {'......Movement Rate + '..MouvementRateBonus..' %', Color.AEON})
	...
	for _, Category in UnitTypes do
		...
		table.insert(Tp.Line, {'......Movement Rate for '..ConvertCategory[Category]..' +'..SpecializationBonus..' %', Color.YELLOW1})
	end
	table.insert(Tp.Line, {''})
	if DM.GetProperty(id, 'CurrentBardsong') == 'BalladoftheWind' then
...
function OnCast(unit, TempEntity, Option) -- Mandatory function. This function is called from ui immediately when the power icon is clicked on
	...
	if DM.GetProperty(id, 'CurrentBardsong') ~= 'BalladoftheWind' then
		...
		DM.SetProperty(id, 'CurrentBardsong', 'BalladoftheWind')
...
BardSongBuff = function(unit, Specialization)
	...
	repeat
		...
		local TechLevel = CF.GetUnitTech(unit)
		local MouvementRateBonus = math.min(math.ceil(Power), 33)
		local SpecializationBonus = MouvementRateBonus
		local time = math.floor(GetGameTimeSeconds())
				...
				if not DM.GetProperty(_id, 'EffectTime_'..Name(unit)) and _unit.IsBuilt == true then	
					for _,category in Specialization do
						if table.find(_bp.Categories, category) then
							SpecializationBonus = math.ceil(MouvementRateBonus * 1.5)
						end
					end
					DM.SetProperty(_id, 'EffectTime_'..Name(unit), 1)
					CreateLightParticleIntel(_unit, -1, _army, 2, 5, 'glow_02', 'ramp_Yellow_01' )
					_unit[Name()..'MouvementRateBuffThread'] = _unit:ForkThread(MouvementRateBuff, SpecializationBonus)
					_unit[Name()..'KillPowerThread'] = _unit:ForkThread(KillPower, 4.9, Name(unit))
				end
			end
		end
		WaitSeconds(5)
	until(unit.Dead == true or DM.GetProperty(id, 'CurrentBardsong') ~= 'BalladoftheWind')
end

MouvementRateBuff = function(unit, MouvementRateBonus)
	local bp = unit:GetBlueprint()
	BuffBlueprint {
		Name = 'BalladoftheWind',
		DisplayName = 'BalladoftheWind',
		BuffType = 'Bardsong',
		Stacks = 'REPLACE',
		Duration = 5,
		Affects = {
			MoveMult = {
				Add = 0,
				Mult = (1 + (MouvementRateBonus/100)),
			},
		},
	}
	Buff.ApplyBuff(unit, 'BalladoftheWind')
end

PersonalBuff = function(unit, MouvementRateBonus)
	local id = unit:GetEntityId()
	repeat
 		local bp = unit:GetBlueprint()
		unit:CreateFx(ModPath..'Graphics/Emitters/BalladoftheWind.bp', 0, 1, 1, 'CreateFxOnBones')
		BuffBlueprint {
			Name = 'BalladoftheWind',
			DisplayName = 'BalladoftheWind',
			BuffType = 'Bardsong',
