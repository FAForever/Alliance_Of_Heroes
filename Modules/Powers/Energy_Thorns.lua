...
local BCMod = {Fighter = 0.5, Rogue = 1, Support = 0.25, Ardent = 1}
local PCMod = {Elite = 0.25, Guardian = 0.25, Dreadnought = 0.25, Bard = 0.25, Ranger = 1}


function Name(unit) -- Mandatory function
	return 'Energy Thorns'
end
...
function Description(unit) -- Mandatory function
	...
	local Power = math.pow(bp.Economy.BuildCostMass, 0.7) * (1 + Int/15)* (1 + Rangercraft/100) * PowerModifier * 2
	Power = math.ceil(Power * RankLevel)
	local StrikeToShield = math.ceil(Power * 2.5)
	local SnareLevel = math.ceil(20 * RankLevel * PowerModifier * (1 + Rangercraft/100))
	local Techlevel = CF.GetUnitTech(unit)
	table.insert(Tp.Line, {'Energy Thorns'})
	table.insert(Tp.Line, {'Rank : '..CF.GetRankName(RankLevel), Color.GREY_LIGHT})
	table.insert(Tp.Line, {'Single target damage overtime and snare', Color.WHITE})
	table.insert(Tp.Line, {''})
	table.insert(Tp.Line, {'Damage strike', Color.WHITE})
	table.insert(Tp.Line, {'Artillery', Color.PURPLE})
	table.insert(Tp.Line, {'Damage : '..Power, Color.CYBRAN})
	table.insert(Tp.Line, {'Damage overtime: '..Power*15, Color.CYBRAN})
	...
	table.insert(Tp.Line, {'Snare : -'..SnareLevel..' %', Color.CYBRAN})
	table.insert(Tp.Line, {''})
...
function GetPowerCost(unit) -- Mandatory function
	...
	local Rangercraft = 0
	if DM.GetProperty(id, 'Rangercraft', 0) then Rangercraft = CF.GetSkillCurrent(id, 'Rangercraft') end
	return math.ceil(70 * RankLevel * (1 + Int/75) + Rangercraft/250)
end
...
function OnWeaponHit(target, instigator) -- Mandatory if this function is called when the unit hit the target (active if DM.SetProperty(id, 'ExecuteWeaponBuffOnTarget', Name(unit)) has been set in OnCast function)
	if instigator and target then
		...
		if CF.GetUnitTech(instigator) >= CF.GetUnitTech(target) and not table.find(targetbp, 'AIR') then
			...
			local RankLevel = ((BCMod[BaseClass] or 0.25) +( PCMod[PrestigeClass] or 0.25)) / 2
			local Rangercraft = 0
			if DM.GetProperty(idi, 'Rangercraft', 0) then Rangercraft = CF.GetSkillCurrent(idi, 'Rangercraft') end
			local PowerModifier = CF.GetStanceModifier(instigator, 'PowerStrengh_Mod') + (DM.GetProperty(idi, 'Buff_PowerDamage_ALL_Add', 0) / 100)
			local SnareLevel = math.max(100 - (20 * RankLevel * PowerModifier * (1 + Rangercraft/100)), 0)
			BuffBlueprint {
				Name = 'Ranger Snare',
				DisplayName = 'Ranger Snare',
				BuffType = 'Moving Buff',
				Stacks = 'STACKS',
				Duration = GetDuration(instigator),
				Affects = {
						MoveMult = {
							Add = 0,
							Mult = SnareLevel / 100,
			...
			Buff.ApplyBuff(target, 'Ranger Snare')
			CreateLightParticleIntel(target, -1, target:GetArmy(), 2, 10, 'glow_02', 'ramp_white_01' )
		end
	end
end
...
function StrikeData(unit) -- Mandatory if this function is called. When DM.SetProperty(id, 'ExecuteStrikeAtBone', Name(unit)) is set on Oncast function. It will change a projectile bp and cast a strike on the next weapon fire.
	...
	StrikeData.DamageRadius = 0
	StrikeData.ShieldDamageMod = 2.5
	StrikeData.DoTTime = 60
	StrikeData.DoTPulses = 15
	StrikeData.DamageFriendly = false
	StrikeData.CollideFriendly = false
	StrikeData.DamType = 'Artillery'
...
function GetDuration(unit) -- Mandatory function
	...
	local Rangercraft = 0
	if DM.GetProperty(id, 'Rangercraft', 0) then Rangercraft = CF.GetSkillCurrent(id, 'Rangercraft') end
	return math.ceil(7 * RankLevel * (1 + Rangercraft/100))
end
